...
void machine_free()
{
  if (machine.code) { free(machine.code); machine.code = 0; }
  if (machine.tape) { free(machine.tape); machine.tape = 0; }
}
...
bool run(bool skip)
{
  ...
  while(machine.tapePos >= 0 && machine.codePos < (machine.codeSize-1)){
    ...
    if (machine.code[machine.codePos] == '['){
      ...
      case '+' : { machine.tape[machine.tapePos]++; } break;
      case '-' : { machine.tape[machine.tapePos]--; } break;
      case '>' : { machine.tapePos++; } break;
      case '<' : { machine.tapePos--; } break;
      case '.' : { write(2, &machine.tape[machine.tapePos], 1); } break;
      case ',' : { read(0, &machine.tape[machine.tapePos], 1);  } break;
      }
    }
    machine.codePos++;
  }
